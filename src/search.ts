...
export function findChunk(manifest: Manifest, id: string): Chunk | undefined {
  const exact = manifest.chunks.find((chunk) => chunk.id === id);
  if (exact) return exact;
  const hashMatches = manifest.chunks.filter((chunk) => chunk.sha256.startsWith(id));
  if (hashMatches.length > 1) throw new Error(`ambiguous chunk hash prefix: ${id} matches ${hashMatches.length} chunks`);
